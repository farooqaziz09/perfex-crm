...
if ($is_tenant) {
    ...
    hooks()->add_filter('get_upload_path_by_type', function ($path, $type) {
        if (!empty($path) && str_starts_with($path, FCPATH) && !str_starts_with($path, PERFEX_SAAS_TENANT_UPLOAD_BASE_FOLDER)) {
            $path = str_ireplace(FCPATH, PERFEX_SAAS_TENANT_UPLOAD_BASE_FOLDER, $path);
        }
        return $path;
    }, 10, 2);
}