    ...
    if (perfex_saas_is_tenant()) {
        ...
        if ($tenant) {
            ...
            if ($tenant->status != 'active') {
                perfex_saas_middleware_force_load_lang();
                perfex_saas_show_tenant_error(ucfirst(_l('perfex_saas_' . $tenant->status)), _l('perfex_saas_company_not_active_mid') . ' <a class="text-blue-600 text-bold tw-font-bold" href="' . perfex_saas_default_base_url('clients/tickets') . '">' . _l('perfex_saas_clich_here') . '</a>', 400);
            }
