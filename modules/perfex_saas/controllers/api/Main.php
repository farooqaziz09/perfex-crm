...
trait Main
{
     ...
     * @return mixed
     */
    private function api_key_middleware($method = '')
    {
        $this->api_enabled_middleware();

        // @todo Control endpoint accessibility using the calling function
        // $api_method = debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS, 2)[1]['function']??null;

        $authorization = $this->input->get_request_header("Authorization", true);
        $token = get_option('perfex_saas_api_key');

        if (empty($token) || strlen($token) < 32) {
            return $this->response_json(['error' => $token . _l('perfex_saas_api_key_invalid')], 402);
        }
