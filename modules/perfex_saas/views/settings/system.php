                ...
                <div class="panel">
                    <div class="panel-body">
                        <div class="input-group">
                            <input type="text" placeholder="<?= _l('perfex_saas_purchase_code'); ?>"
                                class="form-control" id="purchase-code" data-value="<?= $purchase_code; ?>"
                                data-action="<?= admin_url(PERFEX_SAAS_ROUTE_NAME . '/system/save_purchase_code'); ?>"
                                value="<?= $purchase_code; ?>" readonly>
                            <span class="input-group-btn">
                                <button class="btn btn-primary purchase_code_update" id="edit-button"
                                    data-toggle="tooltip" data-title="<?= _l('edit'); ?>"><i
                                        class="fa fa-pen"></i></button>
                                <button class="btn btn-success purchase_code_update" style="display: none;"
                                    id="save-button" data-toggle="tooltip" data-title="<?= _l('save'); ?>"><i
                        ...
                        <div class="text-center tw-mt-5">
                            <div class="tw-flex tw-justify-between">
                                <div
                                    class="tw-text-lg <?= $installed_version == $latest_version ? 'text-success' : 'text-danger'; ?>">
                                    <?= _l('your_version'); ?>:
                            ...
                            <div class="tw-text-2xl tw-w-full tw-mt-4">
                                ...
                                <a onclick="javascript: return confirm('<?= _l('perfex_saas_backup_warning'); ?>');"
                                    href="<?= admin_url(PERFEX_SAAS_ROUTE_NAME . '/system/get_module/' . PERFEX_SAAS_MODULE_NAME); ?>"
                                    class="btn btn-<?= $installed_version != $latest_version ? 'danger' : 'primary'; ?> btn-block"><?= $installed_version != $latest_version ? _l('update_now') : _l('download'); ?></a>
            ...
            <div class="panel-body">
                ...
                <div class="tw-grid tw-gap-3 tw-grid-cols-1 sm:tw-grid-cols-2 md:tw-grid-cols-3 tw-mt-4">
                    ...
                    <div class="panel_s tw-p-4 tw-py-2 tw-bg-neutral-100">
                        <div
                            class="panel_body tw-flex tw-flex-col tw-items-center tw-justify-center text-center tw-gap-3 tw-h-full tw-relative">
                            <div class="tw-text-2xl">
                                <?= $module->name; ?>
                                <?php if (!empty($module->tag)) : ?>
                                <span class="badge bg-success tw-text-xs tw-px-1 tw-absolute tw-right-0"
                                    <?php if (!empty($module->tag_hint)) : ?> data-toggle="tooltip"
                                    data-title="<?= $module->tag_hint; ?>" <?php endif; ?>>
                                    <?= $module->tag; ?>
                                </span>
                                <?php endif; ?>
                            </div>
                            ...
                            <div
                                class="tw-text-xl tw-w-full tw-mt-4 tw-grid tw-gap-3 tw-grid-cols-<?= $installed_module ? '2' : '1'; ?>">

                                <a onclick="javascript: return confirm('<?= _l('perfex_saas_backup_warning'); ?>');"
                                    href="<?= !empty($module->link) ? $module->link : admin_url(PERFEX_SAAS_ROUTE_NAME . '/system/get_module/' . $module_name); ?>"
                                    class="btn btn-<?= $installed_module && $installed_version != $module->version ? 'danger' : 'primary'; ?> btn-block"><?= $installed_module && $installed_version != $module->version ? _l('update_now') : _l('download'); ?></a>

                                <?php if ($installed_module) : ?>
                                <?php if ($installed_module['activated']) : ?>
                                <a href="<?= admin_url(PERFEX_SAAS_ROUTE_NAME . '/system/deactivate/' . $module_name); ?>"
                                    class="btn btn-danger"><?= _l('module_deactivate'); ?></a>
                                <?php else : ?>
                                <a href="<?= admin_url(PERFEX_SAAS_ROUTE_NAME . '/system/activate/' . $module_name); ?>"
                                    class="btn btn-success"><?= _l('module_activate'); ?></a>
                                <?php endif; ?>
                                <?php endif; ?>
                            </div>
                        </div>
