...
if (isset($invoice)) {
    $_duedate = \Carbon\Carbon::parse($invoice->duedate);
    $now = \Carbon\Carbon::now();
    if ($_duedate->gt($now)) {
        $days_left = $on_trial ? (int)$_duedate->diffInDays() : '';
        $invoice_days_left = $_duedate->diffInDays($now, true);
    }
}
                ...
                <div class="summary-card">
                    ...
                    <div class="tw-flex tw-flex-col tw-gap-2 tw-justify-end tw-w-full">
                        ...
                        <button class="btn btn-success tw-w-full"
                            onclick="return confirm('<?= _l('perfex_saas_confirm_customize_package'); ?>');"><?= _l('perfex_saas_subscribe'); ?></button>
                        <?php endif; ?>
